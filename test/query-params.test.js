...
test('[transforms] throws if passing negative numbers to pixel-params', t => {
  t.throws(() => mapQueryParameters({w: '-130'}), /positive number/)
  t.throws(() => mapQueryParameters({h: '-1'}), /positive number/)
  t.end()
})
...
test('[transforms] throws if source rectangle does not have 4 integers', t => {
  t.throws(() => mapQueryParameters({rect: '800'}), /x,y,w,h/)
  t.end()
})

test('[transforms] throws if crop is not positional', t => {
  t.throws(() => mapQueryParameters({crop: 'foo'}), /one of/)
  t.end()
})
