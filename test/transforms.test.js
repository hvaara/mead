...
test('translates `w` param into numeric `width`', t => {
  const {width} = validate({w: '1024'})
  t.equal(width, 1024)
  t.end()
})

test('skips unknown params', t => {
...
test('throws if quality is out of range', t => {
  t.throws(() => validate({q: '800'}), /be between/)
  t.end()
})

test('includes all passed, recognized props in output', t => {
  const input = {
    w: '13',
    h: '14',
    q: '15',
